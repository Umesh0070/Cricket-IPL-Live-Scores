...
var uniqueId = "1216502"; // required ipl match unique id from cric api
const url =  "https://cricapi.com/api/cricketScore?apikey=" +Your_API_Key +"&unique_id=" + uniqueId;
const matchUrl = "https://cricapi.com/api/matches?apikey=" + Your_API_Key;
...
var timer = setInterval(() => {
  fetch(url).then((response) => {
    response.json().then((data) => {
      console.log(data);
      data["team-1"].includes("Hyderabad") ||
      data["team-2"].includes("Hyderabad")
        ? (document.getElementById("team1-logo").src =
            "assets/images/srh_logo.png")
        : "false";
      data["team-1"].includes("Delhi") || data["team-2"].includes("Delhi")
        ? (document.getElementById("team2-logo").src =
            "assets/images/dc_logo.png")
        : "false";
      data["team-1"].includes("Mumbai") || data["team-2"].includes("Mumbai")
        ? (document.getElementById("team1-logo").src =
            "assets/images/mi_logo-.png")
        : "false";
      data["team-1"].includes("Bangalore") ||
      data["team-2"].includes("Bangalore")
        ? (document.getElementById("team2-logo").src =
            "assets/images/rcb_logo-.png")
        : "false";
      data["team-1"].includes("Chennai") || data["team-2"].includes("Chennai")
        ? (document.getElementById("team1-logo").src =
            "assets/images/csk_logo-.png")
        : "false";
      data["team-1"].includes("Kolkata") || data["team-2"].includes("Kolkata")
        ? (document.getElementById("team2-logo").src =
            "assets/images/kkr_logo-.png")
        : "false";
      data["team-1"].includes("Punjab") || data["team-2"].includes("Punjab")
        ? (document.getElementById("team1-logo").src =
            "assets/images/kxip_logo-.png")
        : "false";
      data["team-1"].includes("Rajasthan") ||
      data["team-2"].includes("Rajasthan")
        ? (document.getElementById("team2-logo").src =
      ...
      if (data.error) {
        ...
        } else if (onlyScore[3] == null) {
          team2Wick.textContent = "";
        } else {
        ...
        fetch(matchUrl).then((response) => {
          response.json().then((data) => {
            ...
            let reqMatch = allMatches.find(
              (x) => x.unique_id === parseInt(uniqueId)
            );
            date = new Date(reqMatch.dateTimeGMT);
            stats.textContent = "Match starts at " +date.getHours() +":" +date.getMinutes() +" IST Stay Tuned for Live Scores !";
          });
        });
